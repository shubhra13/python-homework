...
#This holds two different projects called `PyBank` and `PyRamen` description below:

---
## `PyBank`

In this Python script we are analyzing the financial records of your company. This dataset is composed of two columns, Date and Profit/Losses.
...
Resulting analysis should looks like the following:
...
* Then, loop through every row in the `sales` list object.
  ...
  * Performs a quick check if the `sales_item` is already included in the `report`. If not, initialize the key-value pairs for the particular `sales_item` in the report. Then, set the `sales_item` as a new key to the `report` dictionary and the values as a nested dictionary containing the following:
...
* The contents of the `report` dictionary to a text file. as shown:
